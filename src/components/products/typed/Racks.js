...
const Racks = React.memo((props) => {
        ...
        async function fetchInitData() {
            ...
            await fetch(process.env.REACT_APP_NKS_API + 'products/filtersAll', {
                ...
                .then((filters) => {
                    ...
                    console.log('Не удалось получить список фильтров для стеллажей');
                });

            // TODO заменить на слэш! /products -> и в остальных местах тоже
            await fetch(process.env.REACT_APP_NKS_API + 'racks', {
            })
                .then(res => res.json())
                .then((products) => {
                    setProductsData(products);
                }, (error) => {
                    console.log('Не удалось получить стеллажи');
                })
    ...
    const getProducts = async (filtersQueryParams) => {
        ...
        await fetch(process.env.REACT_APP_NKS_API + `racks?${filtersQueryParams}`)
            .then(res => res.json())
            .then((products) => {
                setProductsData(products);
                return products;
