...
const Stands = React.memo((props) => {
        ...
        async function fetchInitData() {
            ...
            await fetch(process.env.REACT_APP_NKS_API + 'products/filtersAll', {
                ...
                .then((filters) => {
                    ...
                    console.log('Не удалось получить список фильтров для стоек');
                });

            // TODO заменить на слэш! /products -> и в остальных местах тоже
            await fetch(process.env.REACT_APP_NKS_API + 'stands', {
            })
                .then(res => res.json())
                .then((products) => {
                    setProductsData(products);
                }, (error) => {
                    console.log('Не удалось получить стойки');
                })
    ...
    const getProducts = async (filtersQueryParams) => {
        ...
        await fetch(process.env.REACT_APP_NKS_API + `stands?${filtersQueryParams}`)
            .then(res => res.json())
            .then((products) => {
                setProductsData(products);
                return products;
